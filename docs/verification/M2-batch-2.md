...
### Re-check (after `0da2ade` "data(media): fix lead images flagged in M2-06 check")

Re-verified all 7 flagged files independently — re-queried the Commons API (`imageinfo`/`extmetadata`/`categories`) for every new file and downloaded/viewed each new lead's thumbnail directly, rather than trusting the new captions.

- **`thessalonica`** — `-01` is now `Thessaloniki_Ancient_Agora_(1).jpg`. Confirmed: `license`/`licenseUrl`/`author` all match Commons exactly (CC BY-SA 4.0, Armineaghayan); `url` resolves; thumbnail shows genuine agora ruins; caption is neutral and accurate. **Pass.**
- **`berea`** — `-01` is now `Βέροια,_Ημαθίας.jpg` (CC BY-SA 4.0, Apaleutos25 — matches Commons exactly). Thumbnail confirmed: a genuine aerial view of the town of Veria. `-02` keeps the "Saint Paul altar" shrine, now honestly captioned as a memorial shrine rather than "street view"/"architecture." Since the file now has a real cityscape lead, this is an acceptable secondary image. **Pass.**
- **`corinth`** — `-01` is now `Temple_d'Apollon_à_Corinthe.jpg` (CC0, Van Der Meulen Christofle — matches Commons exactly). Thumbnail (already downloaded in the original check) is a genuine photo of the standing Temple of Apollo columns, matching its caption. **Pass.**
- **`colossae`** — `-01` is now `Colossae'den_Honaz_görünüm.jpg` (CC BY-SA 4.0, Gülhan Nurhan — matches Commons exactly). Thumbnail (already downloaded) is a genuine landscape photo from the site toward Mount Honaz. The old decorative map image was removed from the file entirely rather than kept as a later index. **Pass.**
- **`gethsemane`** — `-01` is now `Getsemane2.JPG` (CC BY-SA 3.0, Mewasul — matches Commons exactly; Commons category "Olive trees in Gethsemane"). Thumbnail confirmed: gnarled olive trees in a walled garden, matching the caption. `-02` is `Church_of_all_Nations.jpg` (`Public domain` per Commons `LicenseShortName`, categorized `PD-author`; the record's `licenseUrl` uses the same generic `Commons:Public_domain` link the project already uses elsewhere for self-released PD files, e.g. `derbe`). Thumbnail confirmed: the church's mosaic façade. **Pass**, both images.
- **`cana`** — single image, now `IHM_פיצול_עם_שביל_כחול_לחורבת_קנה_1.jpeg` (CC BY-SA 4.0, author "דותן דני" — matches Commons exactly; category `Khirbet Qana`). Thumbnail confirmed: a hillside/trail landscape near the site. Its caption ("Landscape at Khirbet Qana, one candidate site... identification not settled") is appropriately hedged given `cana`'s multiple-candidate status. **Pass.**
- **`lystra` — ruling: remove `data/media/lystra.json`.** The file's only image, `Saint_Paul_at_Lystra_MET_DP820060.jpg` (Jacob Jordaens), remains unchanged: Commons' own `ObjectName`/categories confirm it is a "Drawing" — a narrative painting of the sacrifice-at-Lystra scene (Acts 14), not a photo of the place, exactly as flagged in the original check. Independently confirmed no substitute exists: Commons' `Category:Lystra` contains only planthopper-insect files, and a direct Commons search for "Hatunsaray Lystra" returns zero results. **Wikidata data-quality issue confirmed and corrected in detail**: Wikidata's `Q535982` (labeled "Lystra" in English) carries `P373` (Commons category) = `"Kilistra"` and `P18` (image) = `C+B-Galatia-Map.JPG` (a regional map, not a Lystra photo) — its coordinate (37.664119°N, 32.210711°E) matches the separate, correctly identified Wikidata item `Q85774370` "Kilistra, ancient city of Lycaonia" (37.66472°N, 32.21053°E) almost exactly, confirming `Q535982` conflates Lystra with Kilistra. However, **the real distance is ~14.5 km, not ~30 km** as the fix commit states: `lystra.json`'s own Pleiades-sourced candidate (Tel Lystra near Hatunsaray, 37.588278°N, 32.344473°E) is ~14.5 km from Kilistra's coordinate by direct haversine calculation — matching this same location record's own pre-existing note ("about 14 km northeast... appears to be a coordinate error"), confirmed in an earlier fact-check round. This is a correction to the commit message's distance figure, not to the underlying Wikidata-conflation finding, which stands. Per the card's rule ("if no suitable free image exists, keep the location record and omit only its media file; the human decides at CP2"), **`data/media/lystra.json` has been removed** (by this fact-check, as instructed); `data/locations/lystra.json` is untouched. `npm test` (46/46) and `npm run validate:data` (pass, same 95 pre-existing warnings) both stay green after the removal — the validator does not require every location to have a media file. Total images: 82 → 79 (Media Curator's first pass) → 77 (7-file fix) → **76** (this removal).

**Verdict: all 7 needs-change items resolved.** 6 are fixed leads (all confirmed pass); 1 (`lystra`) is resolved by removing its media file per the card's documented fallback rule. **M2-06 is now Verified.**
