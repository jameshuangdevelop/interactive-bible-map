# M2-02 — Core sites, batch 1: Jerusalem, Judea and Galilee (20 locations)
...
## Goal
Write, illustrate and verify the first 20 of the 40 core MVP sites, so that the M3 app has real, cited and verified data to show.
...
| Group | ids | Notes |
|---|---|---|
| Regions | `judea`, `galilee`, `samaria` | `zoomTier: region`; parents of the places below |
...
| Judea | `bethlehem`, `bethany`, `jericho`, `emmaus` | For `emmaus`, give every serious candidate site. |
| Galilee | `nazareth`, `capernaum`, `bethsaida`, `cana`, `magdala`, `sea-of-galilee` | For `bethsaida` and `cana`, give every serious candidate site. |
...
- Captions are neutral and factual. A region may use a public-domain or freely licensed map. If no suitable free image exists, leave the location out and list it in the PR body.
...
- [ ] All 20 locations exist and validate. Each is `verified`, or is listed in the verification report with a reason and has the human's OK to move to `BACKLOG.md`.
- [ ] Every disputed site (`golgotha`, `emmaus`, `bethsaida`, `cana`) lists every serious candidate, with support and sources, in neutral wording.
...
- [ ] Every image license is on the accepted list and was confirmed on its Commons file page.
